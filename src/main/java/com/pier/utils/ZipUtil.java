...
public class ZipUtil {
    ...
    /**
     * 使用给定密码压缩指定文件或文件夹到指定位置.(每次只压缩单个文件)
     * @param file
     * @param dest
     * @param passwd
     */
    public static String zipPdf(File file, String dest, String passwd) {
        dest = buildDestinationZipFilePath(file, dest);
        ZipParameters parameters = new ZipParameters();
    ...
    /**
     * 使用给定密码压缩指定文件或文件夹到指定位置.(每次只压缩单个文件,不加密)
     * @param file
     * @param dest
     */
    public static String zipSingle(File file, String dest, String p1, String p2, String p3) {
        dest = buildDestinationZipFilePath(file, dest);
        ZipParameters parameters = new ZipParameters();
        parameters.setCompressionMethod(CompressionMethod.DEFLATE);
        parameters.setCompressionLevel(CompressionLevel.NORMAL);
        try {
            ...
            new ZipFile(dest).addFiles(filesToAdd, parameters);
        } catch (ZipException e) {
            log.error("zip error", e);
        }
        return dest;
